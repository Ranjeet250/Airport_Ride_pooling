...
class PoolMatcher {
    ...
    static async match(rideRequestId) {
        ...
        return lockManager.withLock(LOCK_KEYS.POOL_MATCHING, async () => {
            ...
            const originLat = request.originLat;
            const originLng = request.originLng;
            ...
            for (const pool of openPools) {
                ...
                const candidateRoute = RouteService.optimizeRoute(candidateStops, originLat, originLng);
                ...
                if (allTolerancesOk) {
                    const newDirectDist = RouteService.directDistance(newStop.lat, newStop.lng, originLat, originLng);
            ...
            if (bestPool) {
                ...
                const newRoute = RouteService.optimizeRoute([newStop], originLat, originLng);
            ...
            const directDist = RouteService.directDistance(newStop.lat, newStop.lng, originLat, originLng);
            const detourKm = bestRoute
                ? Math.max(
                    0,
                    RouteService.routedDistanceForPassenger(bestRoute.order, newStop, originLat, originLng) -
                    directDist
                )
                : 0;
