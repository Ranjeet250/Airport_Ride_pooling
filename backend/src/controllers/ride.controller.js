...
const { REQUEST_STATUS, RIDE_DIRECTION } = require('../utils/constants');
    ...
    static async requestRide(req, res, next) {
        try {
            const {
                ...
                destLng,
                destAddress,
                direction = RIDE_DIRECTION.FROM_AIRPORT,
                luggageCount = 1,
                maxDetourRatio = 1.4,
            } = req.body;
            ...
            const rideRequest = await prisma.rideRequest.create({
                data: {
                    passengerId,
                    originLat: originLat ? parseFloat(originLat) : 12.9941,
                    originLng: originLng ? parseFloat(originLng) : 80.1709,
                    destLat: parseFloat(destLat),
                    destLng: parseFloat(destLng),
                    destAddress,
                    direction: direction || RIDE_DIRECTION.FROM_AIRPORT,
    ...
    static async getRideStatus(req, res, next) {
        try {
            ...
            return res.status(200).json({
                ...
                direction: rideRequest.direction,
