...
| Database | PostgreSQL |
| ORM | Prisma |
| Cache/Locking | In-memory mutex (Redis-compatible interface) |
...
1. **Single airport origin** — All rides originate from Chennai International Airport (12.9941, 80.1709). The system can be configured for any airport via `src/config/index.js`.
2. **Max 4 passengers per pool** — Vehicle seat limit constrains pool size, making brute-force route optimization O(4!) = O(24) feasible.
3. **Haversine distance** — Straight-line GPS distance is used as a proxy for road distance. In production, integrate Google Maps or OSRM for actual road routing.
4. **Stateless matching** — Each ride request is matched independently. The system does not batch or delay requests to find better pools.
5. **In-memory concurrency** — For local development, Redis and BullMQ are replaced with in-memory equivalents (same interface). Production deployment should swap to real Redis + BullMQ.
6. **No authentication** — API endpoints are unauthenticated for demonstration. Production should add JWT/OAuth.
7. **Synchronous pool matching** — The in-memory queue processes jobs immediately. Production BullMQ provides true async, distributed processing.
...
> **Note:** Redis is NOT required for local development. The system uses an in-memory replacement with the same interface.
...
│   └── DESIGN.md                  # LLD: class diagram, patterns, DSA
├── tests/
│   ├── route-service.test.js      # Route optimization tests
│   ├── pricing-engine.test.js     # Pricing formula tests
│   ├── lock-manager.test.js       # Concurrency lock tests
│   └── queue-service.test.js      # Queue processing tests
├── src/
│   ├── index.js                   # Express app entry point
│   ├── config/
│   │   ├── index.js               # App configuration
│   │   ├── prisma.js              # PrismaClient singleton
│   │   └── redis.js               # In-memory cache (Redis interface)
│   ├── controllers/
...
│   │   ├── lock-manager.js        # In-memory mutex locks
│   │   └── queue.service.js       # In-memory async queue
│   ├── workers/
│   │   └── pool.worker.js         # Queue consumer
│   ├── middleware/
...
After seeding, start the server and the worker will automatically process the pending requests, grouping compatible passengers into pools.
...
| Suite | Tests | Covers |
|-------|-------|--------|
| `route-service.test.js` | 12 | Haversine, route cost, optimization, permutations |
| `pricing-engine.test.js` | 9 | Fare calculation, pool discount, demand surge, detour penalty |
| `lock-manager.test.js` | 5 | Acquire/release, serialization, error recovery |
| `queue-service.test.js` | 5 | Enqueue, health metrics, completion, failure, cleanup |
...
- Design patterns used (Singleton, Producer-Consumer, Strategy, Mutex, Optimistic Locking, Template Method)
- DSA approach with complexity analysis
- Concurrency model with sequence diagram
